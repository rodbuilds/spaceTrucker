...
- FROM: `Requirement: Faction Commodity Reports merchant availability`
- TO:   `Requirement: Faction Survey purchase available at Trading Posts and Headquarters`

- FROM: `Requirement: Report purchase grants codex artifact`
- TO:   `Requirement: Faction Survey purchase grants codex artifact`

- FROM: `Requirement: Report pricing scales by faction characteristics`
- TO:   `Requirement: Faction Survey pricing scales by Specialization`

- FROM: `Requirement: Codex display of archetype and bias summary`
- TO:   `Requirement: Stored Faction Survey content is minimal and live-rendered`

- FROM: `Requirement: Reports are non-transferable in v1`
- TO:   `Requirement: Faction Surveys are non-transferable`
...
**Reason:** The vanilla API `Faction:getRelationsStatuses` is not exposed in the targeted Avorion build. An alternative implementation using `Galaxy():getFactionRelationStatus` against every other faction is prohibitively expensive to run per Codex render. Removing the requirement until a viable API surface is available is honest about current capability.

**Migration:** None for players (the badge never reliably worked in MVP). Implementation removes the placeholder code that stubbed the call. A future change can re-introduce this capability if the API exposure changes.
