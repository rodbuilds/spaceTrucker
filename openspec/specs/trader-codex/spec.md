# trader-codex Specification

## Purpose

Provide a client-side Trader's Codex window, accessed from the player menu, that lists each Faction Survey the player has acquired and renders a per-faction detail view (Economy, Specialization, commodity-bias table) live from the player's current state — without any station references, which live in the Quantum Trading AI Trade Report.
